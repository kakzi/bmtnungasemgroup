...
use App\Filament\Resources\QardhUjrotulBilAmalResource\Pages;
use Illuminate\Database\Eloquent\SoftDeletingScope;
use App\Filament\Resources\QardhUjrotulBilAmalResource\RelationManagers;
use Livewire\Features\SupportFileUploads\TemporaryUploadedFile;

class QardhUjrotulBilAmalResource extends Resource
{
    ...
    protected static ?string $navigationLabel = 'Akad Qardh Ujrotul Bil Amal';
    ...
    public static function form(Form $form): Form
    {
        return $form
            ->schema([
                Section::make('Data Anggota Qardh Ujrotul Bil Amal')
                ->columns([
                    'sm' => 2,
                    'xl' => 12,
                    '2xl' => 12,
                ...
                ->schema([
                    ...
                    Select::make('approvement_id')
                        ...
                        ->afterStateUpdated(function (Set $set, $state) {
                            ...
                            $noUrutAkhir = Agreement::where('akad', 'qard-ujrotul-bil-amal')
                                ...
                                ->whereHas('permohonan', function ($query) {
                                        $query->where('status_permohonan', 'approved')
                                            ->whereHas('pembiayaan', function ($query) {
                                                $query->where('slug', 'qard-ujrotul-bil-amal'); // Filter by pembiayaan name
                                            });
                                })
                                ->get()
                ...
                ->schema([
                    FileUpload::make('file_agreement')
                        ...
                        FileUpload::make('file_foto_akad')
                            ...
                            ->directory('dokumentasi-qubil-amal')
                            ->columnSpan([
                                'sm' => 2,
                                'xl' => 6,
                                '2xl' => 6,
    ...
    public static function table(Table $table): Table
    {
        return $table
            ...
            ->modifyQueryUsing(function (Builder $query) { 
                ...
                $check = User::with('roles')->where('id', $id)->first();
                if($check->roles[0]->name == 'Staff') {
                    return $query->where('akad', 'qard-ujrotul-bil-amal');
                } else if($check->roles[0]->name == 'Branch Manager') {
                    return $query->where('office_id', auth()->user()->office_id)->where('akad', 'qard-ujrotul-bil-amal');
                } else if($check->roles[0]->name == 'Regional Manager') {
                    return $query->where('office_id', auth()->user()->office_id)->where('akad', 'qard-ujrotul-bil-amal');
                } else if($check->roles[0]->name == 'Manager') {
                    return $query->where('akad', 'qard-ujrotul-bil-amal');
                    ...
                    return $query->where('office_id', auth()->user()->office_id)->where('make_by', auth()->user()->name)->where('akad', 'qard-ujrotul-bil-amal');
    ...
    {
        return [
            'index' => Pages\ListQardhUjrotulBilAmals::route('/'),
            'create' => Pages\CreateQardhUjrotulBilAmal::route('/create'),
            'edit' => Pages\EditQardhUjrotulBilAmal::route('/{record}/edit'),
        ];
    }
}
