...
use App\Filament\Resources\MurabahahDPResource\Pages;
use Livewire\Features\SupportFileUploads\TemporaryUploadedFile;
use App\Filament\Resources\MurabahahDPResource\RelationManagers;

class MurabahahDPResource extends Resource
{
    ...
    protected static ?string $navigationLabel = 'Akad Murabahah DP';
                ...
                ->schema([
                    ...
                    Select::make('approvement_id')
                        ...
                        ->afterStateUpdated(function (Set $set, $state) {
                            ...
                            $noUrutAkhir = Agreement::where('akad', 'murabahah-dp')
                                ...
                                ->whereHas('permohonan', function ($query) {
                                        $query->where('status_permohonan', 'approved')
                                            ->whereHas('pembiayaan', function ($query) {
                                                $query->where('slug', 'murabahah-dp'); // Filter by pembiayaan name
                                            });
                                })
                                ->get()
                    ...
                    Select::make('angsuran')
                        ...
                        ->afterStateUpdated(function (Get $get, Set $set, $state) {
                            ...
                            if ($angsuran) {
                                ...
                                // Calculate total_mrbh dp
                                $totalBeli = ((int) str_replace(',', '', $get('harga_beli') ?? 0) - (int) str_replace(',', '', $get('dp_mrbh') ?? 0));
                                $totalUjroh = ((int)$totalBeli  * $upUjroh / 100) * $bulan;
                                $hargaJual = ($totalUjroh + (int) str_replace(',', '', $get('harga_beli') ?? 0));

                                $plafondPembiayaan = ((int) $hargaJual - (int) str_replace(',', '', $get('dp_mrbh') ?? 0));      

                                $angsuranPokokBulan = ((int)$totalBeli / $bulan);
                                $ujrohPerBulan = ((int)$totalBeli * $upUjroh / 100);
                                ...
                                $set('total_mrbh', $plafondPembiayaan);
                ...
                ->schema([
                    FileUpload::make('file_agreement')
                        ...
                        FileUpload::make('file_foto_akad')
                            ...
                            ->directory('dokumentasi-murabahah-dp')
                            ->columnSpan([
                                'sm' => 2,
                                'xl' => 6,
                                '2xl' => 6,
    ...
    public static function table(Table $table): Table
    {
        return $table
            ...
            ->modifyQueryUsing(function (Builder $query) { 
                ...
                $check = User::with('roles')->where('id', $id)->first();
                if($check->roles[0]->name == 'Staff') {
                    return $query->where('akad', 'murabahah-dp');
                } else if($check->roles[0]->name == 'Branch Manager') {
                    return $query->where('office_id', auth()->user()->office_id)->where('akad', 'murabahah-dp');
                } else if($check->roles[0]->name == 'Regional Manager') {
                    return $query->where('office_id', auth()->user()->office_id)->where('akad', 'murabahah-dp');
                } else if($check->roles[0]->name == 'Manager') {
                    return $query->where('akad', 'murabahah-dp');
                    ...
                    return $query->where('office_id', auth()->user()->office_id)->where('make_by', auth()->user()->name)->where('akad', 'murabahah-dp');
    ...
    {
        return [
            'index' => Pages\ListMurabahahDPS::route('/'),
            'create' => Pages\CreateMurabahahDP::route('/create'),
            'edit' => Pages\EditMurabahahDP::route('/{record}/edit'),
        ];
    }
}
