...
use App\Filament\Resources\QardhIjarahResource\Pages;
use Illuminate\Database\Eloquent\SoftDeletingScope;
use App\Filament\Resources\QardhIjarahResource\RelationManagers;
use Livewire\Features\SupportFileUploads\TemporaryUploadedFile;

class QardhIjarahResource extends Resource
{
    ...
    protected static ?string $navigationLabel = 'Akad Qardh Ijarah';
    ...
    public static function form(Form $form): Form
    {
        return $form
            ->schema([
                Section::make('Data Anggota Qardh Ijarah')
                ->columns([
                    'sm' => 2,
                    'xl' => 12,
                    '2xl' => 12,
                ...
                ->schema([
                    ...
                    Select::make('approvement_id')
                        ...
                        ->afterStateUpdated(function (Set $set, $state) {
                            ...
                            $noUrutAkhir = Agreement::where('akad', 'qardh-ijarah')
                                ...
                                ->whereHas('permohonan', function ($query) {
                                        $query->where('status_permohonan', 'approved')
                                            ->whereHas('pembiayaan', function ($query) {
                                                $query->where('slug', 'qardh-ijarah'); // Filter by pembiayaan name
                                            });
                                })
                                ->get()
                ...
                ->schema([
                    FileUpload::make('file_agreement')
                        ...
                        FileUpload::make('file_foto_akad')
                            ...
                            ->directory('dokumentasi-qardh-ijarah')
                            ->columnSpan([
                                'sm' => 2,
                                'xl' => 6,
                                '2xl' => 6,
    ...
    public static function table(Table $table): Table
    {
        return $table
            ...
            ->modifyQueryUsing(function (Builder $query) { 
                ...
                $check = User::with('roles')->where('id', $id)->first();
                if($check->roles[0]->name == 'Staff') {
                    return $query->where('akad', 'qardh-ijarah');
                } else if($check->roles[0]->name == 'Branch Manager') {
                    return $query->where('office_id', auth()->user()->office_id)->where('akad', 'qardh-ijarah');
                } else if($check->roles[0]->name == 'Regional Manager') {
                    return $query->where('office_id', auth()->user()->office_id)->where('akad', 'qardh-ijarah');
                } else if($check->roles[0]->name == 'Manager') {
                    return $query->where('akad', 'qardh-ijarah');
                    ...
                    return $query->where('office_id', auth()->user()->office_id)->where('make_by', auth()->user()->name)->where('akad', 'qardh-ijarah');
    ...
    {
        return [
            'index' => Pages\ListQardhIjarahs::route('/'),
            'create' => Pages\CreateQardhIjarah::route('/create'),
            'edit' => Pages\EditQardhIjarah::route('/{record}/edit'),
        ];
    }
}
