...
use App\Filament\Resources\RahnResource\Pages;
use Illuminate\Database\Eloquent\SoftDeletingScope;
use App\Filament\Resources\RahnResource\RelationManagers;
use Livewire\Features\SupportFileUploads\TemporaryUploadedFile;

class RahnResource extends Resource
{
    ...
    protected static ?string $navigationLabel = 'Akad Rahn';
    ...
    public static function form(Form $form): Form
    {
        return $form
            ->schema([
                Section::make('Data Anggota Rahn')
                ->columns([
                    'sm' => 2,
                    'xl' => 12,
                    '2xl' => 12,
                ...
                ->schema([
                    ...
                    Select::make('approvement_id')
                        ...
                        ->afterStateUpdated(function (Set $set, $state) {
                            ...
                            $nominal_acc = Approvement::whereIn('office_id', (array) auth()->user()->office_id)
                            ...
                            $noUrutAkhir = Agreement::where('akad', 'rahn')
                                ...
                                ->whereHas('permohonan', function ($query) {
                                        $query->where('status_permohonan', 'approved')
                                            ->whereHas('pembiayaan', function ($query) {
                                                $query->where('slug', 'rahn'); // Filter by pembiayaan name
                                            });
                                })
                                ->get()
    ...
    public static function table(Table $table): Table
    {
        return $table
            ...
            ->modifyQueryUsing(function (Builder $query) { 
                ...
                $check = User::with('roles')->where('id', $id)->first();
                if($check->roles[0]->name == 'Staff') {
                    return $query->where('akad', 'rahn');
                } else if($check->roles[0]->name == 'Branch Manager') {
                    return $query->whereIn('office_id', (array) auth()->user()->office_id)->where('akad', 'rahn');
                } else if($check->roles[0]->name == 'Regional Manager') {
                    return $query->whereIn('office_id', (array) auth()->user()->office_id)->where('akad', 'rahn');
                } else if($check->roles[0]->name == 'Manager') {
                    return $query->where('akad', 'rahn');
    ...
    {
        return [
            'index' => Pages\ListRahns::route('/'),
            'create' => Pages\CreateRahn::route('/create'),
            'edit' => Pages\EditRahn::route('/{record}/edit'),
        ];
    }
}
