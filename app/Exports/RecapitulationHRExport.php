...
class RecapitulationHRExport implements FromCollection, WithHeadings, WithStyles
{
    ...
    public function collection()
    {
        ...
        $allUsers = User::select(
                'users.id', 'users.name', 'offices.name as office_name', 'roles.name as role_name'
            )
            ...
            ->leftJoin('attendances', function ($join) use ($date) {
                $join->on('users.id', '=', 'attendances.user_id')
                    ->whereBetween('attendances.date_absensi', [
                        Carbon::parse($date->date_start)->startOfDay(),
                        Carbon::parse($date->date_start)->endOfDay()
                    ]);
            })
            ...
            ->leftJoin('point_lkh_santris', function ($join) use ($date) {
                $join->on('users.id', '=', 'point_lkh_santris.user_id')
                    ->whereBetween('point_lkh_santris.created_at', [
                        Carbon::parse($date->date_start)->startOfDay(),
                        Carbon::parse($date->date_start)->endOfDay()
                    ]);
            })
            ->selectRaw('
                COUNT(DISTINCT cutis.id) AS total_accepted_cuti,
                COUNT(DISTINCT izins.id) AS total_accepted_izin,
                COALESCE(SUM(CASE WHEN detail.type = "in" THEN detail.point ELSE 0 END), 0) AS total_points_datang,
                COALESCE(SUM(CASE WHEN detail.type = "out" THEN detail.point ELSE 0 END), 0) AS total_points_pulang,
                COALESCE(SUM(CASE WHEN karakter.type = "tilawah" THEN karakter.poin ELSE 0 END), 0) AS total_points_tilawah,
                COALESCE(SUM(CASE WHEN karakter.type = "tahajud" THEN karakter.poin ELSE 0 END), 0) AS total_points_tahajud,
                COALESCE(SUM(point_lkh_santris.point_lkh), 0) AS total_points_lkh,
                COALESCE(SUM(point_lkh_santris.point_kehadiran), 0) AS total_points_kehadiran
            ')
        ...
        $mergedData = $allUsers->map(function ($user) use ($date) {
            return [
                ...
                'Kehadiran %' => $date->active_day > 0 
                    ...
                    : '0%',
                'Izin' => $user->total_accepted_izin ?? '0',
                'Cuti' => $user->total_accepted_cuti ?? '0',
            ];
        });

        RecapitulationHR::create([
            'note' => "Rekapitulasi HR Santri (" . Carbon::parse($date->date_end)->format('F Y') . ")",
    ...
    {
        return [
            ['DATA REKAPITULASI SANTRI KSPPS BMT NU NGASEM'],
            [
                'Nama Santri', 'Kantor', 'Jabatan', 'Total Hari', 'Hari Aktif',
                'Datang Pulang', '%', 'Tilawah', '%', 'Tahajud', '%',
                'LKH', '%', 'Kehadiran', '%', 'Izin', 'Cuti'
            ],
        ];
    }
...
//         // Merge the reports
//         $mergedData = $allUsers->map(function ($user) use ($date) {
//             dd($user->total_points_datang + $user->total_points_pulang);
